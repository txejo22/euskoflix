...
public class ModeloPersonas {
	...
	public void crearModeloPersona(HMIntegerDouble pHMMatrizVal, HMStringDouble pHMModeloProducto, Double pUmbral) throws IOException {
		...
		for (Map.Entry<?, ?> entry : pHMMatrizVal.entrySet()) { //por cada usuario
			...
			for (Map.Entry<?, ?> entry2 : ((HashMap<Integer, Double>) entry.getValue()).entrySet()) { //por cada pelicula
	...
	private void sumar(HMStringDouble pHMModeloProductoReducido, int pUserId) {
		...
		for (Map.Entry<?, ?> entry : pHMModeloProductoReducido.entrySet()) { //por cada pelicula
			...
			for (Map.Entry<?, ?> entry2 : ((HashMap<String, Double>) entry.getValue()).entrySet()) { //por cada etiqueta
	...
	public void crearSimilitud(HMStringDouble pHMModeloProducto, HMStringDouble pModeloPersona){
	...
	public TableModel toTableRdo(Map<?,?> pVector) {
		...
		for (Map.Entry<?, ?> entry : pVector.entrySet()) {
	    		model.addRow(new Object[] { entry.getKey(), MovieTitles.getMovieTitles().buscarPorId((Integer)entry.getKey()) , f.format((Double)entry.getValue()*5.0)});
	    }   
